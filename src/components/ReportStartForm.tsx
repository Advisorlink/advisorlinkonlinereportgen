...
export function ReportStartForm({ prefill }: { prefill: ReportStartPrefill }) {
  ...
  // Reset the non-identity fields whenever we switch to a different client so
  // one client's typed-in answers never bleed into another's profile. The
  // pre-fill basics (name, email, phone, age, fund name, balance) are handled
  // by the effect above; everything else starts blank until the user saves.
  useEffect(() => {
    setAnnualIncome("");
    setRetirementAge("67");
    setGoalBalance("");
    setDesiredIncomeAmount("");
    setDesiredIncomeFrequency("Weekly");
    setPrimaryOption("Growth (Default)");
    setOptions([]);
    setMakesContrib("no");
    setContribAmount("");
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [prefill.clientEmail, prefill.clientPhone, prefill.clientName]);
