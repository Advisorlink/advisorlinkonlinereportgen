...
export function AICallerLeads() {
  ...
  async function selectLead(lead: any) {
    ...
    const hasExtractedFields =
      enrichedLead.extracted_fields &&
      Object.values(enrichedLead.extracted_fields).some(Boolean);
    ...
    if (needsReprocess) {
      ...
      try {
        ...
        toast.error(
          e.message || "Could not extract lead answers from this call",
        );
      } finally {
  ...
  const fieldCards = [
    {
      label: "Super Fund",
      value: ef.super_fund_name || ef.super_fund || ef.fund_name || ef.fund,
      icon: Building,
      color: "text-cyan",
    },
    {
      label: "Balance",
      value: ef.balance || ef.super_balance,
      icon: DollarSign,
    ...
    { label: "Age", value: ef.age, icon: User, color: "text-amber-400" },
    {
      ...
      value: ef.had_review_before || ef.previous_review || ef.review,
      icon: ShieldCheck,
      ...
      <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
        ...
        <div className="flex items-start gap-4">
          <Button
            variant="ghost"
            size="icon"
            className="rounded-full mt-1 shrink-0 hover:bg-navy/10"
            onClick={() => {
        ...
        <div className="flex gap-2 flex-wrap">
          {Object.entries(statusConfig).map(([status, config]) => (
            ...
            >
              <span
                className={`inline-block w-1.5 h-1.5 rounded-full mr-1.5 ${selectedLead.status === status ? config.dot : "bg-muted-foreground/30"}`}
              />
              {config.label}
            </button>
          ))}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {fieldCards.map((fc) => (
            <Card
              key={fc.label}
              className="bg-card border-border hover:border-cyan/20 transition-colors"
            >
        ...
        {extractingLeadId === selectedLead.id && (
          <div className="flex items-center gap-2 rounded-xl border border-cyan/15 bg-cyan/5 px-4 py-3 text-sm text-muted-foreground">
            ...
            <span>
              Reading the call transcript and filling the client’s answers…
            </span>
          </div>
        )}
        ...
        {(() => {
          ...
          const extra = Object.entries(ef).filter(
            ...
            {!recordingUrl && (
              <Card className="bg-card border-border">
                <CardContent className="p-5 flex items-center gap-3 text-sm text-muted-foreground">
                  ...
                  <span>
                    {extractingLeadId === selectedLead.id
                      ? "Checking the call log for the recording…"
                      : "No browser-playable recording was saved for this lead."}
                  </span>
                </CardContent>
              </Card>
            )}
                ...
                <Card className="bg-card border-border overflow-hidden">
                  <CollapsibleTrigger asChild>
                    <button className="w-full px-5 py-4 flex items-center justify-between hover:bg-muted/20 transition-colors">
                      <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                        <FileText className="w-4 h-4 text-cyan" /> Full
                        Transcript
                      </h3>
                      <ChevronDown
                        className={`w-4 h-4 text-muted-foreground transition-transform duration-200 ${transcriptOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                  </CollapsibleTrigger>
                  <CollapsibleContent>
            ...
            <Card className="bg-card border-border overflow-hidden sticky top-4">
              <div className="px-5 pt-5 pb-3 border-b border-border bg-gradient-to-r from-navy/5 to-transparent">
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                  ...
                  <span className="ml-auto text-[10px] bg-cyan/10 text-cyan px-2 py-0.5 rounded-full font-bold">
                    {notes.length}
                  </span>
                </h3>
              </div>
                ...
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {notesLoading ? (
                    ...
                    notes.map((note) => (
                      <div
                        key={note.id}
                        className="bg-muted/30 rounded-lg p-3 space-y-1 border border-border/50"
                      >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-foreground">
                        {Math.floor(selectedLead.call_duration_seconds / 60)}m{" "}
                        {selectedLead.call_duration_seconds % 60}s
                      </p>
                    </div>
                  </div>
                </CardContent>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {leads.length} leads generated from AI calls
          </p>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2.5 mb-1">
                        ...
                        <span
                          className={`text-[10px] px-2.5 py-0.5 rounded-full font-semibold ${sc.bg} ${sc.text} flex items-center gap-1`}
                        >
                          <span
                      ...
                      <div className="flex gap-4 text-xs text-muted-foreground">
                        ...
                        {lead.call_duration_seconds != null && (
                          <span className="flex items-center gap-1">
                            ...
                            {Math.floor(lead.call_duration_seconds / 60)}:
